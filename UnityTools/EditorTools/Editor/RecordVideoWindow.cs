...
namespace EditorTools
{
	...
	{
		const string HasInitializedPrefsKey = "EditorTools.RecordVideoWindow.HasInitialized";
		const string RecordHotkeyPrefsKey = "EditorTools.RecordVideoWindow.RecordHotkey";
		const string FFmpegPathPrefsKey = "EditorTools.RecordVideoWindow.FFmpegPath";
		const string OutputFolderPrefsKey = "EditorTools.RecordVideoWindow.OutputFolder";
		const string SuperSizePrefsKey = "EditorTools.RecordVideoWindow.SuperSize";
		const string FrameratePrefsKey = "EditorTools.RecordVideoWindow.Framerate";
		const string OutputFormatPrefsKey = "EditorTools.RecordVideoWindow.OutputFormat";
		const string DeleteFramesPrefsKey = "EditorTools.RecordVideoWindow.DeleteFrames";
		...
		[MenuItem( "Window/Pixelated/Editor Tools/Record Video")]
		...
		string FFmpegPathButtonText()
		{
			...
			return path.Length == 0 ? "Pick FFmpeg Path" : "FFmpeg Path: " + path;
		}

		string OutputFolderButtonText()
		{
			bool pathNotSet = _recordVideo.folderPath == null || _recordVideo.folderPath.Length == 0;
			return pathNotSet ? "Pick Output Folder" : "Output Folder: " + _recordVideo.folderPath;
		}
